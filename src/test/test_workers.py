...
async def test_worker_initialization():
    worker = Worker(topic_name="my_queue", concurrency=3, rate_limit={"max": 5, "duration": 15000}, processor_callback=process_job)
...
async def test_worker_process_task_with_retry():
    ...
    worker = Worker(topic_name="my_queue", concurrency=3, rate_limit={"max": 5, "duration": 15000}, processor_callback=process_job_with_error)
    await worker.connect()
    try:
...
async def test_worker_process_task_with_timeout():
    ...
    worker = Worker(topic_name="my_queue", concurrency=3, rate_limit={"max": 5, "duration": 2000}, processor_callback=process_job_with_timeout)
    await worker.connect()
    try:
        sub = await worker.get_subscriptions()
        msgs = await worker.fetch_messages(sub[0])
...
async def test_worker_fetch_retry():
    ...
    try:
        ...
        assert task_name == ["my_queue.task_1.1", "my_queue.task_2.1", "my_queue.task_3.1"]

    ...
    async def start(self):
        ...
        while True:
            for sub in subscriptions:
                try:
                    msgs = await sub.fetch(self.concurrency, timeout=5)  # Increase timeout if needed
                    if msgs:
                        for msg in msgs:
                            asyncio.create_task(self._process_task(msg))
                    else:
                        await asyncio.sleep(1)  # Sleep to avoid busy-waiting when no messages are available
                    break  # Если задачи найдены, приоритетный pull завершён
                except Exception as e:
                    logger.error(f"Ошибка получения сообщений: {e}")
                    await asyncio.sleep(1)  # Backoff before retrying to avoid busy loops

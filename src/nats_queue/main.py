...
class Job:
    def __init__(self, queue_name, name, data, timeout=None, delay=0, meta=None):
        ...
        self.meta = meta or {
            "retry_count": 0,
            "start_time": (datetime.now() + timedelta(milliseconds=self.delay)).isoformat(),
    ...
    async def _process_task(self, msg):
        try:
            ...
            if job_start_time > datetime.now():
                ...
                logger.info(f"Время для задачи {job_data['name']} еще не наступило. Повторная отправка через {delay}.")
                self.active_tasks-=1
            ...
            logger.error(f"Ошибка при обработке задачи {job_data['name']} истек timeout: {e}", exc_info=True)
            job_data['meta']["retry_count"] += 1
            job = Job(queue_name=job_data["queue_name"], name=job_data["name"], data=job_data["data"], meta=job_data["meta"])
            job_bytes = json.dumps(job.to_dict()).encode()
            await msg.term()
            await self.js.publish(msg.subject, job_bytes, headers={"Nats-Msg-Id": job.id})
        except Exception as e:
...
async def main():
    try:
        ...
        jobs1 = [Job(queue_name="my_queue", name=f"task_{i}", data={"key": f"value_{i}"}, timeout=10) for i in range(1, 8)]
        jobs2 = [Job(queue_name="my_queue", name=f"task_{i}", data={"key": f"value_{i}"}, timeout=10) for i in range(8,13)]
